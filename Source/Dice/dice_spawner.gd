...
func _update_dice_queue():
	...
	for n in range(len(dice_queue)):
		dice_queue[n].desired_position = global_position + Vector2((n+1) * dice_queue_spacing, 0)
...
func _process(delta: float) -> void:
	if dice_to_spawn > 0 and time_since_last_spawn >= time_between_spawns:
		...
		new_dice.position = global_position + Vector2(len(dice_queue) * dice_queue_spacing, 0) + Vector2(200, 0)
		new_dice.desired_position = global_position + Vector2(len(dice_queue) * dice_queue_spacing, 0)
